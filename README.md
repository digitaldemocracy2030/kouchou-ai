...
レポート生成が完了すると、入力データが10,000件以下の場合は自動的に静的ファイルが生成されます。
生成された静的ファイルは管理画面からダウンロードできます。

#### 手動生成

静的ファイルを手動で生成する場合は、以下のコマンドを実行してください。
...
管理画面から、NetlifyやVercelなどの外部ホスティングサービスに直接デプロイすることもできます。
詳細は[外部ホスティングサービスとの連携](./docs/external-hosting.md)を参照してください。
