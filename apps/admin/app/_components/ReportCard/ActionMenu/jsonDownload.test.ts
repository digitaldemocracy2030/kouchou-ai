...
describe("jsonDownload", () => {
  ...
  it("JSONダウンロードが正常に実行され、適切なデータを返す", async () => {
    ...
    const result = await jsonDownload("test-slug");

    expect(global.fetch).toHaveBeenCalledWith("http://localhost:8000/admin/reports/test-slug/json", {
      headers: {
    ...
    expect(result).toEqual({
      ...
      filename: "kouchou_test-slug.json",
