...
def hierarchical_merge_labelling(config: dict) -> None:
    ...
    merge_path = PIPELINE_DIR / f"outputs/{dataset}/hierarchical_merge_labels.csv"
    clusters_df = pl.read_csv(PIPELINE_DIR / f"outputs/{dataset}/hierarchical_initial_labels.csv")
...
def _build_parent_child_mapping(df: pl.DataFrame, cluster_id_columns: list[str]):
    """クラスタ間の親子関係をマッピングする
    ...
    top_cluster_values = df[top_cluster_column].unique().to_list()
    ...
    for idx in range(len(cluster_id_columns) - 1):
        ...
        # 現在のレベルのクラスタid
        current_cluster_values = df[current_column].unique().to_list()
        for current_id in current_cluster_values:
            children_ids = df.filter(pl.col(current_column) == current_id)[children_column].unique().to_list()
...
def melt_cluster_data(df: pl.DataFrame) -> pl.DataFrame:
    ...
    for level in levels:
        ...
        level_unique_val_df = df.select(
            [cluster_columns.id, cluster_columns.label, cluster_columns.description]
        ).unique()
        level_unique_val_df = level_unique_val_df.join(level_count_df, on=cluster_columns.id, how="left")
        level_unique_vals = [
            {
                "level": level,
...
def process_merge_labelling(
    ...
    sampled_data = current_cluster_data.sample(n=sampling_num)
...
def calculate_cluster_density(melted_df: pl.DataFrame, config: dict):
    """クラスタ内の密度計算"""
    hierarchical_cluster_df = pl.read_csv(PIPELINE_DIR / f"outputs/{config['output_dir']}/hierarchical_clusters.csv")

    levels = melted_df["level"].to_list()
    ...
    # 密度のランクを計算
    melted_df = melted_df.with_columns(pl.Series(name="density", values=densities))
    melted_df = melted_df.with_columns(
        pl.col("density").rank(descending=True, method="ordinal").over("level").alias("density_rank")
    )
    melted_df = melted_df.with_columns(
        (pl.col("density_rank") / pl.col("density_rank").count().over("level")).alias("density_rank_percentile")
    )
    return melted_df
