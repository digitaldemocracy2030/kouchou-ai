...
最も確実な方法です。既存リソースを削除する前に、必要なデータは Azure Blob Storage からローカルへ退避してから新規環境を構築します。

```bash
# 1. Azure Blob Storage から既存データをローカルへ退避
...
cd apps/api && rye run python scripts/test_storage.py
```
