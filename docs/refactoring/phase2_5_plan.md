...
> Note
> この文書は planning artifact であり、以下の「現状」は current `main` を表すものではない。2026-05 時点では `analysis-core` は PyPI 公開済みで、`apps/api` も `python -m analysis_core` を subprocess で呼ぶ。
> また branch `codex/refactor-workflow-defaultization` では、CLI canonical path は `run_default()` → `run_workflow()` に寄り、rerun / duplicate reuse / failure semantics の整合もかなり進んでいる。
...
- pyproject.toml 作成済み
- 基本テスト通過 (20 tests)
- `kouchou-ai-analysis-core` の PyPI 公開
- `analysis-core-v*` tag 起点の publish workflow
- `apps/api/src/services/report_launcher.py` からの `python -m analysis_core` 起動

### 未完了の項目
1. **workflow default 化の最終仕上げ**
   - `run_workflow()` の主経路化は進んでいるが、legacy `.run()` と完全同等の運用証拠をまだ詰めている段階
   - e2e / docs / API 運用前提の最終同期が残る

2. **依存関係の軽量化**
   - extras 分割 (`[clustering]`, `[embeddings]`) は未完

3. **deprecated layer の削除**
   - `apps/api/broadlistening/pipeline/` は still present
