...
`public-viewer` を静的エクスポートして Azure Static Web Apps へ載せたい場合は、このページではなく [静的ホスティング向け CSP 設定ガイド](./static-hosting-csp.md) を先に確認してください。静的配信では Plotly の PNG ダウンロードに必要な `img-src blob:` などを、配信先のヘッダー設定で許可する必要があります。
