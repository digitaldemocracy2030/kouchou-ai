...
Plotly の PNG ダウンロードを使う場合や、既に厳しめの CSP を付与している環境へ載せる場合は、`img-src` に `blob:` を含める必要があります。静的エクスポートでは CSP を配信基盤側で設定する前提になるため、詳細は [静的ホスティング向け CSP 設定ガイド](./static-hosting-csp.md) を参照してください。

GitHub Pages 単体は、このガイドで想定しているような柔軟な CSP ヘッダー制御には向きません。CSP を厳密に管理したい場合は、Cloudflare Pages / Azure Static Web Apps / Nginx など、レスポンスヘッダーを設定できる配信先を検討してください。
