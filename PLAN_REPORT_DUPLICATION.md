...
3. UI: 管理画面の複製ダイアログで「Create Report 相当の詳細設定」を編集できるようにする。
   - 編集可能: question / intro / provider / model / prompts（extraction/overview/etc）/ cluster_nums
   - 変更されていない項目は「再利用」表示し、差分のみ overrides として送信する。
...
3. 複製ダイアログで「要約プロンプト」欄を編集する（他の設定は未変更なら再利用）。
...
3. 複製ダイアログの「意見グループ数設定」を変更する (例: `cluster: [5, 10]` の変更)。
...
3. 複製ダイアログの「抽出プロンプト」欄で新しいプロンプトに変更する。
...
- 複製ダイアログ:
  ...
  - クラスタ数
  - model/provider
  - prompts（extraction / initial / merge / overview）
  - 変更されていない項目は「再利用」表示
  - 再利用トグル
    - 再利用トグル:
...
## 次にやること (短期)
- 管理画面の一覧で `source_slug` を表示 (複製アイコン + tooltip + 複製元リンク)。
- 管理画面の `Report` 型に `sourceSlug` を追加して表示に反映。
- 追加したAPI/UI/E2Eのテストを実行し、失敗があれば修正する。
