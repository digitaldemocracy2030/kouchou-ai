...
2. プルリクエストを作成し、変更の詳細を説明してください
  - PR本文は実際の変更内容に合わせてご自身の言葉で簡潔に書いてください。テンプレートやAI生成の文章は参考程度にとどめ、不要な定型セクションは省いて構いません。AIツールの利用自体は歓迎しますが、出力は必ず確認・必要に応じて編集したうえで記載してください。
  - PULL_REQUEST_TEMPLATE.md の各項目（変更概要・背景・関連Issue・動作確認・スクリーンショット・CLA）は実在するリンクと確認結果で埋めてください。
