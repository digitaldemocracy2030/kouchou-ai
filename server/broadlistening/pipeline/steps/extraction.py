...
def extraction(config):
    ...
    for i in tqdm(range(0, len(comment_ids), workers)):
        ...
        batch_results = extract_batch(batch_inputs, prompt, model, workers, provider, config)
...
def extract_batch(batch, prompt, model, workers, provider, local_llm_address=None):
...
def extract_arguments(input, prompt, model, provider, local_llm_address=None):
    messages = [
