...
def hierarchical_initial_labelling(config: dict) -> None:
    ...
    clusters_argument_df = pl.read_csv(f"outputs/{dataset}/hierarchical_clusters.csv")
    ...
    if "cluster_id" in initial_clusters_argument_df.columns:
        initial_clusters_argument_df = initial_clusters_argument_df.drop("cluster_id")
    initial_clusters_argument_df = initial_clusters_argument_df.rename(
        {
...
def initial_labelling(
    ...
    cluster_ids = clusters_df.select(initial_cluster_column).unique().to_series().to_list()
...
def process_initial_labelling(
    ...
    sampling_num = min(sampling_num, cluster_data.height)
    cluster = cluster_data.sample(sampling_num) if sampling_num > 0 else cluster_data
