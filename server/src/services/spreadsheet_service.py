...
def fetch_public_spreadsheet(sheet_id: str, sheet_name: str | None = None) -> pl.DataFrame:
    ...
    try:
        ...
        if "comment-id" not in df.columns:
            generated_ids = [f"id-{i + 1}" for i in range(df.height)]
            df = df.with_columns(pl.Series("comment-id", generated_ids))
...
# def fetch_private_spreadsheet(sheet_id: str, sheet_name: str | None = None, credentials=None) -> pl.DataFrame:
#     """
